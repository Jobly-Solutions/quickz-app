...
export const MapaTiendaRegistro = () => { 
  type Coordinates = {
    loading: boolean;
    location: LatLng;
};
...
const continuar = async (lat:number,long:number) => {  
  try { 
    await AsyncStorage.setItem('@provincia', provincia); 
    await AsyncStorage.setItem('@localidad', localidad); 
    await AsyncStorage.setItem('@direccion', direccion);  
    
  } catch (e) {
    // saving error
  }
  navigation.navigate('CrearTienda',{exito:true,lat:lat,long:long});
};
useEffect(() => { 
  setVisible(true);
  (async () => { 
        ...
        position:'absolute'}}>
            ...
            alignSelf: 'center',}}>
                ...
                <Button
                                    ...
                                    onPressed={() =>  {continuar(location?.latitude,location?.longitude)}}
                                />
